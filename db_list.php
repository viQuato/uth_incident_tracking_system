...
$page->title = "Forms submitted to ". $set->site_name;
...
// sorting
if(!isset($_GET['sort']) || !in_array($_GET['sort'], array(0,1,2)))  // check if it's a valid sort option
  $sort = 0;
else
  $sort = (int)$_GET['sort'];
...
$total_results = mysql_query("SELECT COUNT(*) as count FROM `form_table` ?p", $db)->count) 
    ...
    $data = $db->getAll("SELECT * FROM `form_table` ?p ORDER BY ?p LIMIT ?i,?i", $where, $order_by, $start, $perpage);
    ...
    foreach($data as $u) {
      ...
      <div class='thumbnail clearfix'>
    	<a href='$set->url/profile.php?u=$u->form_id'><img src='".$user->getAvatar($u->userid)."' width='80' alt='".$options->html($u->username)."' class='pull-left clearfix' style='margin-right:10px'>
        <div class='caption' class='pull-left'>
          <h4>      
    	      <a href='$set->url/profile.php?u=$u->form_id'>".$user->showName($u->userid)."</a>
          </h4>
...
<div class='container'>
  ...
  <h3 class='pull-left'>Forms submitted to ".$set->site_name."</h3>
